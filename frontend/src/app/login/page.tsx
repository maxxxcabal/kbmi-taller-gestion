        ...
        <div className="bg-[var(--sidebar)] border border-[var(--border)] rounded-3xl p-8 shadow-[0_20px_50px_rgba(0,0,0,0.5)] backdrop-blur-sm relative overflow-hidden">
          ...
          <div className="space-y-6">
            ...
            <div className="space-y-3 pt-2">
              {/* Acceso Directo Dueño (Bypass Google) */}
              <button 
              ...
              >
                <div className="flex items-center justify-center w-6 h-6 rounded-full bg-[var(--bg)]/10">
                   <ShieldCheck size={14} />
                </div>
                Acceso Directo Dueño
                <ArrowRight size={16} className="ml-auto" />
              </button>

              <div className="flex items-center gap-4 my-6">
                <div className="h-[1px] flex-1 bg-[var(--border2)]" />
                <span className="text-[10px] font-mono text-[var(--text3)] uppercase tracking-widest">Alt.</span>
                <div className="h-[1px] flex-1 bg-[var(--border2)]" />
              </div>
              ...
              <button 
                ...
                className="w-full bg-[var(--bg2)] hover:bg-[var(--card)] text-[var(--text2)] border border-[var(--border)] py-3 px-4 rounded-xl flex items-center justify-center gap-3 font-bold text-xs transition-all active:scale-[0.98] group opacity-70 hover:opacity-100"
              >
                <svg className="w-4 h-4 fill-current group-hover:text-[#4285F4] transition-colors" viewBox="0 0 24 24">
                  <path d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z" />
                  <path d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" />
                  <path d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l3.66-2.84z" />
                  <path d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z" />
                </svg>
                Google (Consola requerida)
              </button>

              {/* Guest / Demo Button */}
              <button 
                onClick={() => signIn('credentials', { callbackUrl: '/dashboard' })}
                className="w-full bg-transparent hover:bg-[var(--bg2)] text-[var(--text3)] text-[10px] font-mono py-2 rounded-lg transition-all opacity-40 hover:opacity-100 uppercase tracking-widest text-center"
              >
